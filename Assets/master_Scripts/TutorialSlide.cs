...
public class TutorialSlide : MonoBehaviour {
    ...
    public Image firstImage;//一枚目のスライド最初に表示される
    ...
    void Start () {
        firstImage.sprite = slideImageSprite[0];//初期画像を一枚目に設定する
        secondImage.sprite = slideImageSprite[0];//初期画像を一枚目に設定する

        if (slideSizeIsScreenSize)
        { slideImageSize = new Vector2(Screen.width,Screen.height); }//画面サイズに合わせる
        if (slidePositionIsCenter)
        { slidePosition = Vector2.zero; }//移動位置を画面中央に設定

        firstImage.rectTransform.localPosition = slidePosition;//設定された位置に移動する
        secondImage.rectTransform.localPosition = slidePosition;//設定された位置に移動する
        firstImage.rectTransform.sizeDelta = slideImageSize;//設定されたスライドのサイズに調節する
    ...
    public IEnumerator FirstPaperSlide(float seconds, bool isSecondSlide)
    {
        ...
        //スライドのサイズと位置を調整
        firstImage.rectTransform.sizeDelta = slideImageSize;
        firstImage.rectTransform.localPosition = new Vector2(0, 0);
        ...
        while (t < 1)
        {
            ...
            {
                //Lerpでスライドする
                firstImage.rectTransform.localPosition = Vector2.Lerp(startSlidePosition, leftSlidePosition, t);
                secondImage.rectTransform.localPosition = Vector2.Lerp(rightSlidePosition, startSlidePosition, t);
            }
            else
            {
                firstImage.rectTransform.localPosition = Vector2.Lerp(startSlidePosition, rightSlidePosition, t);
        ...
        if (isSecondSlide)
        {
            ...
            firstImage.sprite = slideImageSprite[spriteNum];
        }
        else
        {
            spriteNum--;
        ...
        //最終位置を調整する
        firstImage.rectTransform.localPosition = startSlidePosition;
